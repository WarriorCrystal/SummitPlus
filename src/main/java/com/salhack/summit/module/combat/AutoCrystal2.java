...
public class AutoCrystal2 extends Module
{
    ...
    public AutoCrystal2()
    {
        super("AutoCrystal", new String[] {"AutoCrystal2"}, "Automatically places and destroys crystals", "NONE", -1, ModuleType.COMBAT);
    ...
    private AutoMendArmor _autoMend = null;
    private SelfTrap _selfTrap = null;
    private HoleFiller _holeFiller = null;
    ...
    public void init()
    {
        ...
        _autoMend = (AutoMendArmor)ModuleManager.Get().GetMod(AutoMendArmor.class);
        _selfTrap = (SelfTrap)ModuleManager.Get().GetMod(SelfTrap.class);
        _holeFiller = (HoleFiller)ModuleManager.Get().GetMod(HoleFiller.class);
    ...
    private Listener<EventClientTick> OnClientTick = new Listener<>(event ->
    {
        ...
        // override
        if (PlaceMode.getValue() == "Lethal" && _lastPlaceLocation != BlockPos.ORIGIN)
        {
            float damage = 0f;
        ...
        boolean skipUpdateBlocks = _lastPlaceLocation != BlockPos.ORIGIN && PlaceMode.getValue() == "Lethal";
        ...
        if (!skipUpdateBlocks && _remainingTicks <= 0)
        {
            ...
            final List<BlockPos> cachedCrystalBlocks = CrystalUtils.findCrystalBlocks(mc.player, AutoCrystal2.PlaceRadius.getValue()).stream().filter(pos -> VerifyCrystalBlocks(pos)).collect(Collectors.toList());
    ...
    public boolean NeedPause()
    {
        ...
        if (_surround.isEnabled() && !_surround.IsSurrounded() && _surround.HasObsidian())
        {
